...
class FuwuOAuth extends Action
{
	public function index($token)
	{
		...
		if ($_GET['auth_code'] == '') {
			...
			if ($return['alipay_system_oauth_token_response']['access_token'] != '') {
				...
				if (0 < (int) $userinfo_return['error_response']['code']) {
					$this->error('服务窗没有获取粉丝信息权限!<br/>错误码：' . $userinfo_return['error_response']['code'] . '<br/>错误描述：' . $userinfo_return['error_response']['sub_msg']);
					exit('服务窗没有获取粉丝信息权限!<br/>错误码：' . $userinfo_return['error_response']['code'] . '<br/>错误描述：' . $userinfo_return['error_response']['sub_msg']);
				}
