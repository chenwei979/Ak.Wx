...
final class lbsImgNews
{
	...
	public function news($lat, $lon)
	{
		...
		foreach ($news2 as $keya => $infot) {
			$distance = ACOS((SIN(($lat * 3.1415000000000002) / 180) * SIN(($infot['latitude'] * 3.1415000000000002) / 180)) + (COS(($lat * 3.1415000000000002) / 180) * COS(($infot['latitude'] * 3.1415000000000002) / 180) * COS((($lon * 3.1415000000000002) / 180) - (($infot['longitude'] * 3.1415000000000002) / 180)))) * 6380;
