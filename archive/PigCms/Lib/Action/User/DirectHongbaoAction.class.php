...
class DirectHongbaoAction extends UserAction
{
	...
	public function hongbao_add()
	{
		if (IS_POST) {
			...
			$totalmoney = $data['max_money'] * $data['totalnums'];
			...
			if ($data['id'] == '') {
				...
				if ($add) {
					F($this->token . '_' . 'directhongbao', NULL);
	...
	public function send_hongbao()
	{
		...
		$totalmoney = $hongbao['max_money'] * $total_count;
		...
		if (!empty($openid)) {
			...
			if (10 < count($openid)) {
				$send_openids = array_slice($openid, $num * 10, 10);

				if (!empty($send_openids)) {
					foreach ($send_openids as $key => $val) {
						$last_record = M('directhongbao_record')->where(array('hid' => (int) $hongbao['id'], 'fans_id' => $val['openid']))->order('send_time desc')->find();
						if ((0 < $last_record['send_time']) && ((time() - $last_record['send_time']) < 5)) {
							continue;
						}
				...
				$totalnums = ($num * 10) + $successcount;

				if (!$temp) {
					if (($num == 0) && ($successcount == 0)) {
						$this->error($status['msg'], U('DirectHongbao/index', array('token' => $this->token)));
					}
					else {
						M('directhongbao')->where(array('id' => $id))->save(array(
	'totalnums'    => array('exp', 'totalnums+' . $totalnums),
	'lastsendtime' => time(),
	'send_status'  => 1
	));
						$this->error('发送成功了' . $totalnums . '个,但' . $status['msg'] . '退出了发送', U('DirectHongbao/index', array('token' => $this->token)));
					}
				}
				else if (count($openid) <= $num * 10) {
					M('directhongbao')->where(array('id' => $id))->save(array(
	'totalnums'    => array('exp', 'totalnums+' . $totalnums),
	'lastsendtime' => time(),
	'send_status'  => 1
					...
					$this->success('红包批量发送中,请勿关闭浏览器...', U('DirectHongbao/send_hongbao', array('token' => $this->token, 'id' => $id, 'num' => $num + 1)));
					exit();
				}
			}
			else {
	...
	private function request($fansinfo = array(), $params = array())
	{
		if (empty($params) || ($fansinfo['openid'] == '')) {
		...
		if ($params['hb_type'] == 1) {
			...
			return array('status' => 'error.', 'msg' => '发送类型错误');
		}
