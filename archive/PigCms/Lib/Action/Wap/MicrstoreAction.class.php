...
class MicrstoreAction extends WapAction
{
	...
	public function _initialize()
	{
		...
		if (updateSync::getIfWeidian()) {
			$this->Micrstore_URL = C('weidian_domain') ? C('weidian_domain') : 'http://v.wooyx.com';
			$this->SALT = C('encryption') ? C('encryption') : 'qQHJPb6598Z64y72JMx2Z0kp7rpAI5V9';
		}
		else {
			$this->Micrstore_URL = 'http://v.wooyx.com';
			$this->SALT = 'qQHJPb6598Z64y72JMx2Z0kp7rpAI5V9';
		}
	}
	...
	public function api()
	{
		...
		$data = array('store_id' => $store_id, 'wecha_id' => $openid, 'token' => $this->token, 'wechaname' => $userinfo['wechaname'], 'portrait' => $userinfo['portrait'], 'tel' => $userinfo['tel'], 'address' => $userinfo['address'], 'return_url' => $_GET['return_url'] ? $_GET['return_url'] : '');
		$sort_data = $data;
	...
	public function login()
	{
		...
		$userinfo = M('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->find();
		$data = array('store_id' => intval($_GET['store_id']), 'wecha_id' => $this->wecha_id, 'token' => $this->token, 'wechaname' => $userinfo['wechaname'], 'portrait' => $userinfo['portrait'], 'tel' => $userinfo['tel'], 'address' => $userinfo['address'], 'return_url' => $_GET['return_url'] ? $_GET['return_url'] : '');
		$sort_data = $data;
		$sort_data['salt'] = $this->SALT;
		ksort($sort_data);
		$sort_data = array_map('callback', $sort_data);
