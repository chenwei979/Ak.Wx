...
class UnitaryAction extends WapAction
{
	...
	public function cartajax()
	{
		...
		case 'click_cart_count':
			...
			foreach ($cart_list as $vo) {
				$this_order = $this->m_order->where(array('token' => $_POST['token'], 'pigcms_id' => $vo['order_id']))->find();
			...
			if ($cart_count != $_POST['cnum']) {
				...
				foreach ($cart_list as $vo) {
					...
					foreach ($cart_list2 as $vo2) {
						$this_order = $this->m_order->where(array('token' => $_POST['token'], 'pigcms_id' => $vo2['order_id']))->find();
	...
	public function dobuy()
	{
		...
		foreach ($cart_list as $vo) {
			...
			foreach ($cart_list2 as $cvo) {
				$this_order = $this->m_order->where(array('token' => $_POST['token'], 'pigcms_id' => $cvo['order_id']))->find();
				if (($this_order['paid'] != 1) && ($vo['addtime'] < (time() - (60 * 15)))) {
	...
	'addtime'  => array('gt', 1444663403)
		...
		if ($myorder != NULL) {
			$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $myorder['pigcms_id'], 'unitaryid' => $_GET['unitaryid']));
		}
	}
	...
	public function payReturn()
	{
		...
		if ($order) {
			if (1444663403 < $order['addtime']) {
				...
				$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $order['pigcms_id'], 'dopayover' => 1));
			}
			else {
				$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $order['pigcms_id'], 'dopayover' => 1));
			}
	...
	public function payend()
	{
		if (IS_POST) {
			...
			$myorder = M('unitary_order')->where(array('token' => $_POST['token'], 'wecha_id' => $_POST['wecha_id'], 'pigcms_id' => $orderid))->find();
			...
			if ($myorder['addtime'] != 0) {
				...
				if ($cart_list == '') {
					M('unitary_order')->where(array('token' => $_POST['token'], 'wecha_id' => $_POST['wecha_id'], 'pigcms_id' => $orderid))->delete();
				...
				foreach ($cart_list as $key => $val) {
					...
					if (0 < intval($val['count_s'])) {
						...
						$update_order = M('unitary_order')->where(array('token' => $_POST['token'], 'pigcms_id' => $orderid))->save(array('addtime' => 0));
					}
					else {
						$update_order = M('unitary_order')->where(array('token' => $_POST['token'], 'pigcms_id' => $orderid))->save(array('addtime' => 0));
					}
	...
	public function buyres()
	{
		...
		foreach ($cart_list as $k => $vo) {
			...
			$this_order = $this->m_order->where(array('token' => $this->token, 'pigcms_id' => $vo['order_id']))->find();
			if (($this_order['paid'] != 1) && ($vo['addtime'] < (time() - (60 * 15)))) {
				$this->m_cart->where(array('token' => $_POST['token'], 'id' => $vo['id']))->delete();
			}
